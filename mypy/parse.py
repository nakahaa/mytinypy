...
def prefix_nud0(t):
    if check(P.token, 'nl', ';', 'eof', 'dedent'):
        return t
    return prefix_nud(t)
...
def i_infix(bp, led, *vs):
    for v in vs:
        base_dmap[v] = {'lbp': bp, 'bp': bp, 'led': led}
...
def i_terms(*vs):
    for v in vs:
        base_dmap[v] = {'lbp': 0, 'nud': itself}


i_terms(')', '}', ']', ';', ':', 'nl', 'elif', 'else', 'True', 'False',
        'None', 'name', 'string', 'number', 'indent', 'dedent', 'except')
base_dmap['nl']['val'] = 'nl'
