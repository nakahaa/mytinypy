...
int tp_hash(TP,ObjType v) {
    switch (v.type) {
        ...
        case DICTTYPE: return tp_lua_hash(&v.dict.val,sizeof(void*));
        case LISTTYPE: {
            int r = v.list.val->len; int n; for(n=0; n<v.list.val->len; n++) {
            ObjType vv = v.list.val->items[n]; r += vv.type != LISTTYPE?tp_hash(tp,v.list.val->items[n]):tp_lua_hash(&vv.list.val,sizeof(void*)); } return r;
        }
        case FUNCTYPE: return tp_lua_hash(&v.fnc.info,sizeof(void*));
        case DATATYPE: return tp_lua_hash(&v.data.val,sizeof(void*));
    }
